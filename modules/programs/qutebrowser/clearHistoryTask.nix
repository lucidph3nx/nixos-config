{
  ...
  config = lib.mkIf (config.nx.programs.qutebrowser.enable && config.nx.services.qutebrowser.clearHistoryTask) {
    home-manager.users.ben = {
      # a systemd service clear qutebrowser history older than 7 days
      systemd.user.services.clear-qutebrowser-history = let
