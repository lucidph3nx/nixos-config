...
incarnation of that session — so a session that restarted does not leak
the volumes it made before the restart. A resource created before
instance-ID naming carries no token and is swept by the old rule instead
