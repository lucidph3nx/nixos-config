...
in {
  options = {
    ...
    homeManagerModules.wallpaper.resolution = lib.mkOption {
      type = lib.types.str;
      default = "5120x1440";
  ...
  config = lib.mkIf config.homeManagerModules.wallpaper.enable {
    ...
    home.activation.renderWallpaper = lib.hm.dag.entryAfter ["writeBoundary"] ''
      ${pkgs.imagemagick}/bin/convert ${configDir}/wallpaper-${resolution}.svg ${configDir}/wallpaper-${resolution}.png

    '';
    home.file.".config/wallpaper-2880x1800.svg" = lib.mkIf (resolution == "2880x1800") {
      text = let
        colourbg = "${bg_dim}";
        colour1 = "${red}";
        colour2 = "${yellow}";
    ...
    home.file.".config/wallpaper-5120x1440.svg" = lib.mkIf (resolution == "5120x1440") {
      text = let
        colourbg = "${bg_dim}";
        colour1 = "${red}";
        colour2 = "${yellow}";
