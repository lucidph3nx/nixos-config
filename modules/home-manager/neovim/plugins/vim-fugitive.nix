        ...
        */
        ''
          ...
          vim.keymap.set('n', '<leader>gs',
            vim.cmd.Git, { desc = '[G]it [S]tatus in Fugitive' })
          vim.keymap.set('n', '<leader>gmd',
            vim.cmd.Gvdiffsplit, { desc = '[G]it [M]erge [D]iff' })
          vim.keymap.set('n', '<leader>gh',
            '<cmd>diffget //2<CR>', { desc = '[G]it, left side of merge diff' })
          vim.keymap.set('n', '<leader>gl',
            '<cmd>diffget //3<CR>', { desc = '[G]it, right side of merge diff' })
        '';
    }
  ];
}
